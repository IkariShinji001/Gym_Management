...
export class PtController {
  ...
  @Get('/all-id-name')
  async findAllIdAndName(): Promise<Pt[]> {
    return this.ptService.getAllPtWithIdAndName();
  }
  @Get('/profile/:id')
  ...
  async create(@Body() createPtProfilePtDto: CreatePtProfileDto): Promise<Pt> {
    const { createProfileDto, createPtDto, createImagesDto } =
      createPtProfilePtDto;
    return await this.ptService.create(
      createProfileDto,
      createPtDto,
      createImagesDto,
    );
